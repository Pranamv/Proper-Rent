    ...
    <main className="min-h-screen bg-background px-4 py-10 text-foreground">
      <div className="mx-auto max-w-xl rounded-md border border-border bg-surface p-6 shadow-soft">
        ...
        <p className="mt-3 text-sm leading-6 text-muted">
          {authState.status === "forbidden"
            ? "Supabase sign-in succeeded, but the backend did not find a matching admin agent row."
            : authState.status === "misconfigured"
              ? "Set NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY for the frontend environment."
              : "The backend admin auth check could not be reached. Confirm the API service and NEXT_PUBLIC_API_BASE_URL."}
        </p>
        <a
